<#
功能简介：
1) 守护式启动 Phase4 训练，异常自动重试并保留失败记录；
...
param(
    ...
    [string]$FailureLogDir = 'D:/cyd/Desktop/yolo_web-main/experiments/logs/phase4_failures',
...
function Resolve-LatestCheckpoint {
    $latest = Get-ChildItem -Path (Join-Path $Repo 'experiments/YOLOv11-S-MBV3-ECA') -Filter last.pt -Recurse -File |
...
if ($Mode -eq 'resume') {
    if ([string]::IsNullOrWhiteSpace($Checkpoint)) {
        ...
        if ([string]::IsNullOrWhiteSpace($Checkpoint)) {
            throw 'No resumable checkpoint found under experiments/YOLOv11-S-MBV3-ECA'
        }
    }
...
Write-Host '[RESILIENT] Phase4 resilient resume started.'
...
Write-Trianlog -Title 'Phase4 watchdog start' -Lines @(
    "mode=$Mode",
...
for ($attempt = 1; $attempt -le $MaxRestarts; $attempt++) {
    ...
    $cmdArgs = @(
        "$Repo\training\scripts\phase4_train_yolo11s_mbv3_eca.py",
        '--epochs', $Epochs,
        '--workers', $Workers,
        '--batch', $Batch,
        '--cache', $CacheMode,
    ...
    if ($exitCode -eq 0) {
        ...
        Write-Trianlog -Title 'Phase4 watchdog completed' -Lines @("attempt=$attempt", 'exit_code=0')
        exit 0
    }
    ...
    $retryTitle = 'Phase4 watchdog retry'
    if ($exitCode -eq 12) {
        $delay = [Math]::Max($RetryDelaySeconds, $GpuFailureCooldownSeconds)
        $retryTitle = 'Phase4 watchdog gpu-memory retry'
    }
    ...
    if ($Mode -eq 'fresh') {
        ...
        if (-not [string]::IsNullOrWhiteSpace($resumeCkpt) -and (Test-Path $resumeCkpt)) {
            ...
            Write-Trianlog -Title 'Phase4 watchdog switched to resume' -Lines @(
                "attempt=$attempt",
                "checkpoint=$Checkpoint"
            )
        }
...
Write-Trianlog -Title 'Phase4 watchdog failed' -Lines @("max_restarts=$MaxRestarts")
