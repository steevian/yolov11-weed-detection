...
def main() -> int:
    ...
    try:
        # Keep core train kwargs aligned with phase2/phase3 for fair comparison.
        model = YOLO(train_source)
        train_results = model.train(**train_kwargs)
    except KeyboardInterrupt:
        message = "Training interrupted by user/terminal."
        print(f"[WARN] {message}")
        _append_markdown_event(trianlog_path, "Phase4中断", [f"run_id={ctx.run_id}", message])
